...
export function seedDemo() {
  ...
  function ensureUser(u) {
    ...
    if (byUsername[key]) {
      // Do not overwrite existing user details to preserve edits
      return;
    }
  ...
  const doctor1 = byUsername['dranurag'] || { id: uuid() };
  ensureUser({ ...doctor1, username: 'dranurag', firstName: 'Anurag', lastName: 'Regmi', email: 'Anurag.Regmi@pakenham.example', password: 'doctor123', role: 'doctor', specialty: 'Cardiology' });
  const doctor2 = byUsername['drLamichhane'] || { id: uuid() };
  ensureUser({ ...doctor2, username: 'drLamichhane', firstName: 'Manish', lastName: 'Lamichhane', email: 'Manish.Lamichhane@pakenham.example', password: 'doctor123', role: 'doctor', specialty: 'Pediatrics' });
  ...
  ensureUser({ ...admin, username: 'admin', firstName: 'Bipin', lastName: 'Admin', email: 'admin@pakenham.example', password: 'admin123', role: 'admin' });
  ...
  // Ensure a pre-booked appointment exists for tomorrow 10:00 between jdoe and dranurag
  const appts = getAppointments();
  const dr = users.find(u => u.username === 'dranurag');
